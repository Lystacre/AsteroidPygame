...
def displayStartMenu():
    window.fill(SPACE_GREY)
    title = font_title.render('Asteroids', True, LIGHT_BLUE)
    title_width, title_height = font_title.size('Asteroids')
    window.blit(title, ((WINDOW_SIZE[X]- title_width)//2, (WINDOW_SIZE[Y] - title_height)//4))
    message1 = font.render("[Q]uitter", True, LIGHT_BLUE)
    message1_largeur, message1_hauteur = font.size("[Q]quitter")
    window.blit(message1, ((WINDOW_SIZE[X] - message1_largeur) // 2, 4 * WINDOW_SIZE[Y]  // 5))
    message2 = font.render("N'importe quelle touche pour commencer...", True, LIGHT_BLUE)
    message2_largeur, message2_hauteur = font.size("N'importe quelle touche pour commencer...")
